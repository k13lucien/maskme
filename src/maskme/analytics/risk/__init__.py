...
Usage:
    ...
    from maskme.analytics.risk import run, METRICS
    ...
    from maskme.analytics.risk import report
    report.generate(results, output_path="risk_report.html")

Adding a new metric:
    1. Create  risk/metrics/<name>.py  implementing Metric Protocol
...
def run(
    ...
    **kwargs: Any,
    ...
    Returns:
        List of RiskResult instances, one per analytic, in registry
    ...
    selected_names = analytics or list(METRICS.keys())
...
__all__ = ["METRICS", "run", "report", "RiskResult", "Metric"]